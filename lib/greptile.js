...
// Rules in component-map.json are written against the umbrella repo, where the
// Fastify tree lives under `fastify/`. When the PR is on the Fastify fork itself
// the same file arrives as `lib/route.js`, so try that prefix too before giving up.
function mapPath(p) {
  if (!p) return { arch: null, mod: null };
  for (const candidate of [p, `fastify/${p}`]) {
    const rule = MAP.find(r => candidate.startsWith(r.prefix));
    if (rule) return { arch: rule.arch || null, mod: rule.mod || null };
  }
  return { arch: null, mod: null };
}
...
// Rolls files + findings + downstream mentions up to graph nodes at one level
// ('arch' or 'mod'). Shared by the passive PR scrape and the active review.
function aggregateImpact(level, files, findings, downstream = []) {
  const acc = {};
  const touch = id => (acc[id] ??= { files: [], findings: 0, maxSeverity: null, add: 0, del: 0, downstream: false });
  for (const f of files) if (f[level]) {
    const n = touch(f[level]);
    n.files.push(f.path); n.add += f.additions || 0; n.del += f.deletions || 0;
  }
...
async function buildReport(gh, repo, prNumber, prMeta) {
  ...
  return {
    ...
    impact: {
      arch: aggregateImpact('arch', outFiles, findings, downstream),
      mod: aggregateImpact('mod', outFiles, findings, downstream),
    },
    unmapped: outFiles.filter(f => !f.arch).map(f => f.path),
  };
}

module.exports = { makeClient, buildReport, mapPath, aggregateImpact, parseSeverity, readDotEnv, GREPTILE_LOGINS, SEV_ORDER };
